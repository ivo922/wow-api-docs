...
[Source](https://web.archive.org/web/20100726120937/http://wowprogramming.com/docs/widgets/Model)
...
```lua
animationGroup = Model:CreateAnimationGroup(["name" [, "inheritsFrom"]])
```
...
```lua
texture = Model:CreateTexture(["name" [, "layer" [, "inherits"]]])
```
...
```lua
Model:SetBackdropBorderColor(red, green, blue [, alpha])
```
...
```lua
Model:SetHitRectInsets(left, right, top, bottom)
```
