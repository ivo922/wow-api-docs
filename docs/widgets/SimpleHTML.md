...
```lua
animationGroup = SimpleHTML:CreateAnimationGroup(["name" [, "inheritsFrom"]])
```
...
```lua
texture = SimpleHTML:CreateTexture(["name" [, "layer" [, "inherits"]]])
```
...
```lua
SimpleHTML:SetAttribute("name", value)
```
...
```lua
SimpleHTML:SetBackdropBorderColor(red, green, blue [, alpha])
```
