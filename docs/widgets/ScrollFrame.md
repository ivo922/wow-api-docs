...
```lua
animationGroup = ScrollFrame:CreateAnimationGroup(["name" [, "inheritsFrom"]])
```
...
```lua
texture = ScrollFrame:CreateTexture(["name" [, "layer" [, "inherits"]]])
```
...
```lua
ScrollFrame:SetAttribute("name", value)
```
...
```lua
ScrollFrame:SetBackdropBorderColor(red, green, blue [, alpha])
```
