...
```lua
animationGroup = MessageFrame:CreateAnimationGroup(["name" [, "inheritsFrom"]])
```
...
```lua
texture = MessageFrame:CreateTexture(["name" [, "layer" [, "inherits"]]])
```
...
```lua
MessageFrame:SetAttribute("name", value)
```
...
```lua
MessageFrame:SetBackdropBorderColor(red, green, blue [, alpha])
```
...
```lua
MessageFrame:StopMovingOrSizing()
```
...
```lua
MessageFrame:UnregisterAllEvents()
```
...
```lua
MessageFrame:UnregisterEvent("event")
```
