...
```lua
animationGroup = Slider:CreateAnimationGroup(["name" [, "inheritsFrom"]])
```
...
```lua
texture = Slider:CreateTexture(["name" [, "layer" [, "inherits"]]])
```
...
```lua
Slider:SetBackdropBorderColor(red, green, blue [, alpha])
```
