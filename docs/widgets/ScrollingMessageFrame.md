...
[Source](https://web.archive.org/web/20100726120937/http://wowprogramming.com/docs/widgets/ScrollingMessageFrame)
...
- `id` - Identifier for the message's type (see [`:UpdateColorByID()`](ScrollingMessageFrame.md#scrollingmessageframeupdatecolorbyid)) (`number`)
- `addToTop` - True to insert the message above all others listed in the frame, even if the frame's [insert mode](ScrollingMessageFrame.md#scrollingmessageframesetinsertmode) is set to `BOTTOM`; false to insert according to the frame's insert mode (`boolean`)
...
Returns a number identifying the last message added to the frame. This number starts at `0` when the frame is created and increments with each message [AddMessage](ScrollingMessageFrame.md) to the frame; however, it resets to `0` when a message is added beyond the frame's [GetMaxLines](ScrollingMessageFrame.md).
...
```lua
ScrollingMessageFrame:GetDontSavePosition()
```
...
Returns the duration of the fade-out animation for disappearing messages. For the amount of time a message remains in the frame before beginning to fade, see [`:GetTimeVisible()`](ScrollingMessageFrame.md#scrollingmessageframegettimevisible).
...
Scrolls the message frame's contents down by one page. One "page" is slightly less than the [number of lines](ScrollingMessageFrame.md#scrollingmessageframegetnumlinesdisplayed) displayed in the frame.
...
Scrolls the message frame's contents up by one page. One "page" is slightly less than the [number of lines](ScrollingMessageFrame.md#scrollingmessageframegetnumlinesdisplayed) displayed in the frame.
...
```lua
ScrollingMessageFrame:SetDontSavePosition()
```
...
Sets the duration of the fade-out animation for disappearing messages. For the amount of time a message remains in the frame before beginning to fade, see [`:SetTimeVisible()`](ScrollingMessageFrame.md#scrollingmessageframesettimevisible).
...
Enables or disables hyperlink interactivity in the frame. The frame's hyperlink-related [script handlers](#script-handlers) will only be run if hyperlinks are enabled.
...
Sets the amount of time for which a message remains visible before beginning to fade out. For the duration of the fade-out animation, see [`:SetFadeDuration()`](ScrollingMessageFrame.md#scrollingmessageframesetfadeduration).
...
Updates the color of a set of messages already added to the frame. Used in the default UI to allow customization of chat window message colors by type: each type of chat window message (party, raid, emote, system message, etc.) has a numeric identifier found in the global table `ChatTypeInfo`; this is passed as the fifth argument to [`:AddMessage()`](ScrollingMessageFrame.md#scrollingmessageframeaddmessage) when messages are added to the frame, allowing them to be identified for recoloring via this method.
...
- `id` - Identifier for a message's type (as set when the messages were [added](ScrollingMessageFrame.md#scrollingmessageframeaddmessage) to the frame) (`number`)
