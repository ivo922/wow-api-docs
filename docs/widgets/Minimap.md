...
[Source](https://web.archive.org/web/20100726120937/http://wowprogramming.com/docs/widgets/Minimap)
...
```lua
animationGroup = Minimap:CreateAnimationGroup(["name" [, "inheritsFrom"]])
```
...
```lua
fontstring = Minimap:CreateFontString(["name" [, "layer" [, "inherits"]]])
```
...
```lua
texture = Minimap:CreateTexture(["name" [, "layer" [, "inherits"]]])
```
...
```lua
region = Minimap:CreateTitleRegion()
```
...
```lua
Minimap:DisableDrawLayer("layer")
```
...
```lua
depth = Minimap:GetEffectiveDepth()
```
...
```lua
left, right, top, bottom = Minimap:GetHitRectInsets()
```
...
```lua
maxWidth, maxHeight = Minimap:GetMaxResize()
```
...
Returns the location of the last "ping" on the minimap. Coordinates are pixel distances relative to the center of the minimap (not fractions of the minimap's size as with [`:GetPingPosition()`](https://web.archive.org/web/20100726112636/http://wowprogramming.com/docs/widgets/Minimap/GetPingPosition)); positive coordinates are above or to the right of the center, negative are below or to the left.
...
- `zoomLevel` - Index of the current zoom level (between 0 for the widest possible zoom and `(minimap:`[`GetZoomLevels()`](https://web.archive.org/web/20100726112636/http://wowprogramming.com/docs/widgets/Minimap/GetZoomLevels)`- 1)` for the narrowest possible zoom) (`number`)
...
"Pings" the minimap at a given location. Coordinates are pixel distances relative to the center of the minimap (not fractions of the minimap's size as with [`:GetPingPosition()`](https://web.archive.org/web/20100726112636/http://wowprogramming.com/docs/widgets/Minimap/GetPingPosition)); positive coordinates are above or to the right of the center, negative are below or to the left.
...
```lua
Minimap:SetAttribute("name", value)
```
...
```lua
Minimap:SetBackdropBorderColor(red, green, blue [, alpha])
```
...
- `zoomLevel` - Index of a zoom level (between 0 for the widest possible zoom and `(minimap:`[`GetZoomLevels()`](https://web.archive.org/web/20100726112636/http://wowprogramming.com/docs/widgets/Minimap/GetZoomLevels)`- 1)` for the narrowest possible zoom) (`number`)
