...
- `value` - Value indicating the amount of the status bar's area to be filled in (between `minValue` and `maxValue`, where `minValue, maxValue = StatusBar`[`:GetMinMaxValues()`](StatusBar.md#statusbargetminmaxvalues)) (`number`)
...
Sets the color shading for the status bar's texture. As with [`:SetVertexColor()`](LayeredRegion.md#layeredregionsetvertexcolor), this color is a shading applied to the texture image.
...
- `layer` - Graphics layer in which the texture should be drawn; defaults to `ARTWORK` if not specified (`string`, [layer](../types/layer.md))
