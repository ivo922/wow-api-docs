...
[Source](https://web.archive.org/web/20100726120937/http://wowprogramming.com/docs/widgets/StatusBar)
...
```lua
animationGroup = StatusBar:CreateAnimationGroup(["name" [, "inheritsFrom"]])
```
...
```lua
texture = StatusBar:CreateTexture(["name" [, "layer" [, "inherits"]]])
```
...
- `value` - Value indicating the amount of the status bar's area to be filled in (between `minValue` and `maxValue`, where `minValue, maxValue = StatusBar`[`:GetMinMaxValues()`](https://web.archive.org/web/20100726112636/http://wowprogramming.com/docs/widgets/StatuBar/GetMinMaxValues)) (`number`)
...
```lua
StatusBar:SetAttribute("name", value)
```
...
```lua
StatusBar:SetBackdropBorderColor(red, green, blue [, alpha])
```
...
Sets the color shading for the status bar's texture. As with [`:SetVertexColor()`](https://web.archive.org/web/20100726112636/http://wowprogramming.com/docs/widgets/LayeredRegion/SetVertexColor), this color is a shading applied to the texture image.
...
```lua
StatusBar:UnregisterEvent("event")
```
