...
```lua
animationGroup = DressUpModel:CreateAnimationGroup(["name" [, "inheritsFrom"]])
```
...
```lua
texture = DressUpModel:CreateTexture(["name" [, "layer" [, "inherits"]]])
```
...
```lua
depth = DressUpModel:GetEffectiveDepth()
```
...
```lua
scale = DressUpModel:GetEffectiveScale()
```
...
```lua
red, green, blue = DressUpModel:GetFogColor()
```
...
```lua
left, right, top, bottom = DressUpModel:GetHitRectInsets()
```
...
```lua
maxWidth, maxHeight = DressUpModel:GetMaxResize()
```
...
```lua
numChildren = DressUpModel:GetNumChildren()
```
...
```lua
numRegions = DressUpModel:GetNumRegions()
```
...
```lua
DressUpModel:ReplaceIconTexture("filename")
```
...
```lua
DressUpModel:SetAttribute("name", value)
```
...
```lua
DressUpModel:SetBackdropBorderColor(red, green, blue [, alpha])
```
...
```lua
DressUpModel:UnregisterEvent("event")
```
