...
[Source](https://web.archive.org/web/20100726120937/http://wowprogramming.com/docs/widgets/PlayerModel)
...
```lua
animationGroup = PlayerModel:CreateAnimationGroup(["name" [, "inheritsFrom"]])
```
...
```lua
texture = PlayerModel:CreateTexture(["name" [, "layer" [, "inherits"]]])
```
...
```lua
red, green, blue = PlayerModel:GetFogColor()
```
...
```lua
PlayerModel:ReplaceIconTexture("filename")
```
...
```lua
PlayerModel:SetAttribute("name", value)
```
...
```lua
PlayerModel:SetBackdropBorderColor(red, green, blue [, alpha])
```
...
```lua
PlayerModel:StopMovingOrSizing()
```
...
```lua
PlayerModel:UnregisterAllEvents()
```
...
```lua
PlayerModel:UnregisterEvent("event")
```
