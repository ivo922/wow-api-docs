...
Confirms using an item, if using the item causes it to become soulbound. Usable in response to the [`USE_BIND_CONFIRM`](../events/USE_BIND_CONFIRM.md) which fires when the player attempts to use a "Bind on Use" item.
